...
// cost query time. dont use if need to estimate time comsumption
    ...
    void insert_error(int error) {
        // stream frequency estimation error must be non-negative integer
        // assert(error >= 0);
        error_freqs[error]++;
        total_stream_size++;
    }

    void dump_error_distribution() {
        ...
        for (auto &itr: error_freqs) {
            ...
            int error_ratio_to_freq = total_stream_size * error_ratios[current_error_ratio_index];
            sum += itr.second;
            while (itr.first >= error_ratio_to_freq) {
                ...
                error_ratio_to_freq = total_stream_size * error_ratios[current_error_ratio_index];
            }
        }
