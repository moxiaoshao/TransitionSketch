...
//    uint16_t tail_idx;
    ...
    void append_new_key(uint32_t key, int freq) {
        if (now_element < capacity) {
            ...
            //USS
            // uint32_t old_key = tail_node->key;
            // int old_value = tail_node->val;

            // if((double)rand() / RAND_MAX <= 1. / ((double)old_value + 1)){
            //     hash_table.erase(old_key);
            //     tail_node->key = key;
            //     hash_table[key] = tail_node;
            //     add_counter(tail_node, freq);
...
public:
    SpaceSaving(int _capacity) : now_element(0), capacity(_capacity) {
        nodes = new Node[capacity + 1];
        memset(nodes, 0, sizeof(Node) * (capacity + 1));
        now_element = 0;
        nodes[0].val = -1;
...
//        Node *& my_node = hash_table[key];
    ...
    void get_top_k(uint16_t k, uint32_t * result) {
        ...
        void get_top_k_with_frequency(uint16_t k, unordered_map<int, int> & result)
    {
        Node * idx = nodes[0].next;

        int i;
        for (i = 0; i < k && i < capacity && i < now_element; ++i) {
            result[idx->key] = idx->val;
            idx = idx->next;
        }
